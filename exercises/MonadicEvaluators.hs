...
--------------------------------------------------------------------------------
-- | Evaluating Symbolic Expressions, using Maybe for handling

eval :: [(Name,Integer)] -> Expr -> Maybe Integer
eval env (Num n)     = undefined
eval env (Var x)     = undefined
eval env (Add e1 e2) = undefined
eval env (Mul e1 e2) = undefined
eval env (Div e1 e2) = undefined

--------------------------------------------------------------------------------
-- | Monadic Expression Evaluator using the do notation

evalDo :: [(Name,Integer)] -> Expr -> Maybe Integer
evalDo env (Num n)     = undefined
evalDo env (Var x)     = undefined
evalDo env (Add e1 e2) = undefined
evalDo env (Mul e1 e2) = undefined
evalDo env (Div e1 e2) = undefined

--------------------------------------------------------------------------------
-- | Monadic Expression Evaluator using <$> and <*> instead of the do notation

evalA env (Num n)     = undefined
evalA env (Var x)     = undefined
evalA env (Add e1 e2) = undefined
evalA env (Mul e1 e2) = undefined
evalA env (Div e1 e2) = undefined
